...
try:
    ...
    print(a + b) # 에러생성!
    print("아자쓰") # 실행안됨 ㅠ
except TypeError as e: # 에러객체가 e라는 변수에 담김
    print("타입에러!")
    print(f"에러내용: {e}") # 에러객체의 __str__호출

# 3. NameError: 변수 선언전에 읽을 때
# print(name)
# name = "홍길동"

# 4. IndexError: 인덱스 범위를 벗어날 때
my_list = [1,2,3]
# print(my_list[100]) 범위초과

# 5. KeyError: dict에 없는키로 접근할 때
# 6. ValueError: 타입은 맞는데, 값이 이상한경우
try:
    sample = "abc"
    int(sample)
    # int(): 문자열은 맞는데, 값이 숫자처럼 생겨야 됨
    names = ["철수", "영희"]
    names.remove("병철")
